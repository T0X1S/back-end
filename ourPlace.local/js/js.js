...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCountdown() {
        ...
        if (diff <= 0) {
            countdownContainer.innerHTML = 'Акция завершена!';
            return;
        }
        ...
        countdownContainer.innerHTML = `Акция заканчивается через:<br>${hours} ч ${minutes} мин ${seconds} сек`;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    var forms = document.querySelectorAll('form[action*="add_to_cart"]');
    forms.forEach(function(form) {
        form.addEventListener('submit', function(e) {
            ...
            xhr.open('POST', form.getAttribute('action') || 'add_to_cart.php');
            ...
            xhr.onload = function() {
                ...
                try {
                    ...
                    if (data.success) {
                        var cartLink = document.getElementById('header-cart-link');
                        if (cartLink) {
                            cartLink.textContent = data.cart_count > 0 ? 'Корзина (' + data.cart_count + ')' : 'Корзина';
                        }
